...
- **中文自然度：语义正确不等于中文自然。常见中文搭配优先；若句子需要读者先在脑中改写成另一句中文才能顺读，视为 prose defect。**
- **题材语体边界：题材专名可以专业或带时代色彩，普通叙述不得仅因题材自动古文化、公文化、论文式抽象化、翻译化或诗化压缩；没有明确特殊语体时默认自然现代汉语。**
- **策划语泄漏：细纲、Execution Card、审稿或项目管理层的抽象词只传语义，不得成片扩散成叙述者或人物声线。**
- **自然度定级：孤立一两处按局部问题；同类生造搭配、脑内二次翻译、策划腔或过度设计句成片出现并明显增加阅读负担时至少 S2，必须返修，不能因剧情正确而放行。**
...
- 中文搭配风险：句子能解释意思但不像自然中文常见搭配、为了题材或文学感重新组词、普通句子半文半白或持续项目化抽象表达。
- 处理原则：有原文证据才输出 finding；给出可执行替换方向，不只评价“AI 味重”。修法方向不默认「拆短 / 删虚词 / 剥标点」：把正常的逗号长句拆成碎句，与 AI 腔同样是问题。中文自然度问题优先回到普通常用中文，不要求用另一组“漂亮句”替换。
...
5. **形成审查基准包摘要**：把已加载的文件内容或内置 fallback 摘要压缩为 5-12 条审查标准，后续 solo 和子 Agent 都必须使用这份摘要。摘要必须保留句长标准和一条中文自然度标准：常见搭配优先；语义成立但需要脑内二次翻译不算自然；同类自然度问题成片出现按 S2+ 处理。
...
- 提示指令：
  ...
  11. 中文自然度：是否存在生造搭配、翻译式组合、普通句子做旧、诗化压缩、策划/论文/项目管理词扩散成叙述声线，或需要读者脑内二次翻译才能顺读的句子？同类问题成片出现时至少 S2。
...
4. 中文自然度 Gate：检查生造搭配、脑内二次翻译、策划/论文/项目管理语泄漏、题材导致普通句做旧/翻译化/诗化压缩；同类问题成片出现至少 S2。
...
### 中文自然度
- {列出生造搭配、脑内二次翻译、策划语泄漏、普通句做旧/翻译化/诗化压缩问题，必须附 evidence；无则写未发现成片问题}
