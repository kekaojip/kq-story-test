...
Execution Card 的字段值是**策划语义**，不是正文措辞样本，也不是人物或叙述者声线。

Main 与 Writer 都必须遵守：

- 卡片中的分析词、方法词、抽象名词只负责告诉 Writer 当前章“意思和功能是什么”，不得因为这些词出现在卡片里就原样继承到正文；
- Writer 不得把卡片中的抽象框架扩散成整章认知隐喻。例如“杠杆、变量、门槛、压力、控制感、兑现、可验证、现实尺子、资源链”等词，只有角色和场景本身自然会这样说时才进入正文；
- Main 编译卡片时优先用清楚、朴素的当前章描述，避免为了显得专业而把简单剧情改写成咨询、论文、产品或项目管理语言；
- Writer 接到卡片后必须重新用自然中文成文，不能逐字段翻译、换同义词后拼进小说；
- 题材专名、批准台词、必须展示的场内文本除外；这些仍按更高层输入和当前明确要求处理。

判断方式：

> 删除字段名和策划标签后，如果某句话仍像一条“写作分析结论”，而不像人物会经历、看见、想到或说出的东西，就不能直接进入正文。
...
> SEMANTIC ONLY: 本卡所有策划/分析用词只传递意思，不是正文措辞、比喻或叙述声线样本。正文必须重新使用自然中文表达。
...
Main 编译卡片时，可以为准确协作使用少量分析词，但不得把它们当成“推荐正文词汇”。

Writer 看到任何策划表达时执行：

```text
UNDERSTAND MEANING
→ DISCARD PLANNING WORDING
→ WRITE NATURAL PROSE
```
...
```text
EXECUTION CARD WORDING
→ 同义替换
→ 直接进入正文
```

如果一个人物被设定为“实际、会算、重规则”，这只授权人物在当前场景做相应判断，不授权整章旁白持续使用账本、项目管理、流程分析或同一抽象隐喻。
...
- 是否和 00-04 冲突？
- 是否把方法论抄进卡片而不是编译成决定？
- 是否明确保留了 `SEMANTIC ONLY` 措辞隔离？
- 卡片里的策划/分析用语是否过密，容易污染正文声线？
